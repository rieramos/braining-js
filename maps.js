...
wildcards = (function(){ const emojis = { ...symbols }; let wildcards = { 'X':'', 'A':'', }; //vars
  ...
  for (const wildcard in wildcards) {
    switch (wildcard) {
      case 'A': //chooose a posible repeat option or, a new (just to get it some randomness)
                if ( Math.floor(Math.random()*1) === 1) { wildcards[wildcard] = getItem(Object.keys(symbols)); break };

      default: direction = getItem(Object.keys(emojis)); wildcards[wildcard] = direction; delete emojis[direction];
    };
  }; return wildcards;
})();
